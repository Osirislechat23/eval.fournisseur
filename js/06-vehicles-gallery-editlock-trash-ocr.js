  ...
  // ---------- courbe d'évolution du prix du carburant ----------
  const FUEL_CHART_RANGES = [
    { key:'3m', label:'3 mois', months:3 },
    { key:'6m', label:'6 mois', months:6 },
    { key:'1y', label:'1 an', months:12 },
    { key:'all', label:'Tout', months:null }
  ];
  let vehicleFuelRange = '6m';

  function fuelChartSinceDate(){
    const r = FUEL_CHART_RANGES.find(x => x.key === vehicleFuelRange) || FUEL_CHART_RANGES[1];
  ...
  // Pleins avec litres renseignés, triés par date, dans la plage choisie
  function vehicleFuelPoints(v, sinceDate){
    return (v.entries||[])
      .filter(e => e.type === 'carburant' && parseFloat(e.liters) > 0 && parseFloat(e.cost) > 0)
      .map(e => ({
        date: e.date,
        cost: parseFloat(e.cost),
        liters: parseFloat(e.liters),
        km: e.km ? parseInt(e.km,10) : null,
        pricePerLiter: parseFloat(e.cost) / parseFloat(e.liters)
      }))
      .filter(p => !sinceDate || new Date(p.date) >= sinceDate)
      .sort((a,b) => new Date(a.date) - new Date(b.date));
  }

  const FUEL_GRANULARITIES = [
    { key:'day',   label:'Jour' },
    { key:'week',  label:'Semaine' },
    { key:'month', label:'Mois' },
    { key:'year',  label:'Année' }
  ];
  let vehicleFuelGranularity = 'month';

  // Regroupe une date dans sa periode (jour/semaine/mois/annee) : cle de tri + libelle affiche
  function fuelPeriodInfo(dateStr, granularity){
    const d = new Date(dateStr);
    if(granularity === 'day'){
      const key = d.toISOString().slice(0,10);
      return { key, sortDate: new Date(key+'T00:00:00'), label: fmtDate(dateStr) };
    }
    if(granularity === 'week'){
      const mon = mondayOf(d);
      const key = mon.toISOString().slice(0,10);
      return { key, sortDate: mon, label: 'Sem. ' + fmtDate(mon.toISOString()) };
    }
    if(granularity === 'year'){
      const key = String(d.getFullYear());
      return { key, sortDate: new Date(d.getFullYear(), 0, 1), label: key };
    }
    // mois (par defaut)
    const key = d.getFullYear() + '-' + String(d.getMonth()+1).padStart(2,'0');
    return { key, sortDate: new Date(d.getFullYear(), d.getMonth(), 1), label: d.toLocaleDateString('fr-FR', { month:'short', year:'numeric' }) };
  }

  // Agrege les pleins par periode : plus bas, plus haut et moyen sur chaque periode
  function vehicleFuelPeriods(points, granularity){
    const buckets = new Map();
    points.forEach(p => {
      const info = fuelPeriodInfo(p.date, granularity);
      if(!buckets.has(info.key)) buckets.set(info.key, { ...info, prices: [] });
      buckets.get(info.key).prices.push(p.pricePerLiter);
    });
    return Array.from(buckets.values())
      .map(b => ({
        label: b.label, sortDate: b.sortDate,
        min: Math.min(...b.prices), max: Math.max(...b.prices),
        avg: b.prices.reduce((s,v)=>s+v,0) / b.prices.length,
        count: b.prices.length
      }))
      .sort((a,b) => a.sortDate - b.sortDate);
  }

  function renderFuelPriceChartSvg(periods){
    const W = 640, H = 220, padL = 46, padR = 16, padT = 16, padB = 30;
    const innerW = W - padL - padR, innerH = H - padT - padB;
    const allVals = periods.flatMap(p => [p.min, p.max]);
    let lo = Math.min(...allVals), hi = Math.max(...allVals);
    if(lo === hi){ lo -= 0.05; hi += 0.05; }
    const pad = (hi - lo) * 0.12;
    lo -= pad; hi += pad;
    const x = i => padL + (periods.length === 1 ? innerW/2 : (i/(periods.length-1)) * innerW);
    const y = val => padT + innerH - ((val - lo)/(hi - lo)) * innerH;

    const gridCount = 4;
    const gridLines = Array.from({length: gridCount+1}, (_,i) => {
      const val = lo + (hi-lo)*(i/gridCount);
      const yy = y(val);
      return `<line x1="${padL}" y1="${yy.toFixed(1)}" x2="${W-padR}" y2="${yy.toFixed(1)}" stroke="var(--line)" stroke-width="1"/>
        <text x="${padL-6}" y="${(yy+3).toFixed(1)}" text-anchor="end" font-size="10" fill="var(--text-dim)">${val.toFixed(2)}</text>`;
    }).join('');

    const labelIdxs = periods.length <= 6
      ? periods.map((_,i)=>i)
      : [...new Set([0, Math.round((periods.length-1)/3), Math.round((periods.length-1)*2/3), periods.length-1])];
    const periodLabels = labelIdxs.map(i => `<text x="${x(i).toFixed(1)}" y="${H-8}" text-anchor="middle" font-size="10" fill="var(--text-dim)">${esc(periods[i].label)}</text>`).join('');

    const lineOf = (field, color, fieldLabel) => {
      const pts = periods.map((p,i) => `${x(i).toFixed(1)},${y(p[field]).toFixed(1)}`).join(' ');
      const dots = periods.map((p,i) => `<circle cx="${x(i).toFixed(1)}" cy="${y(p[field]).toFixed(1)}" r="3" fill="${color}" stroke="var(--paper-raised)" stroke-width="1.2"><title>${esc(p.label)} — ${fieldLabel} : ${p[field].toFixed(3)} €/L (${p.count} plein${p.count>1?'s':''})</title></circle>`).join('');
      return `<polyline points="${pts}" fill="none" stroke="${color}" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"/>${dots}`;
    };

    return `
      <svg viewBox="0 0 ${W} ${H}" class="fuel-price-svg" role="img" aria-label="Prix du carburant : plus haut, plus bas et moyen par période">
        ${gridLines}
        ${lineOf('max', 'var(--brick)', 'plus haut')}
        ${lineOf('avg', 'var(--gold)', 'moyen')}
        ${lineOf('min', 'var(--sage)', 'plus bas')}
        ${periodLabels}
      </svg>`;
  }

  function renderFuelPriceSection(v){
    const points = vehicleFuelPoints(v, fuelChartSinceDate());
    const rangeSelect = `<select id="vehFuelRange" class="fuel-range-select">${FUEL_CHART_RANGES.map(r => `<option value="${r.key}" ${r.key===vehicleFuelRange?'selected':''}>${r.label}</option>`).join('')}</select>`;
    const granSelect = `<select id="vehFuelGranularity" class="fuel-range-select">${FUEL_GRANULARITIES.map(g => `<option value="${g.key}" ${g.key===vehicleFuelGranularity?'selected':''}>${g.label}</option>`).join('')}</select>`;
    const controls = `<span class="fuel-chart-controls">${granSelect}${rangeSelect}</span>`;
    if(points.length < 2){
      return `
        <div class="dash-section-title fuel-chart-title-row">⛽ Évolution du prix du carburant ${controls}</div>
        <div class="empty-side">Ajoute au moins deux pleins avec le nombre de litres (voir « Ajouter une intervention » ci-dessous) pour voir apparaître la courbe.</div>`;
    }
    const periods = vehicleFuelPeriods(points, vehicleFuelGranularity);
    const overallMin = Math.min(...periods.map(p => p.min));
    const overallMax = Math.max(...periods.map(p => p.max));
    const overallAvg = points.reduce((s,p) => s+p.pricePerLiter, 0) / points.length;
  ...
  function renderVehicleMain(){
    ...
    main.innerHTML = `
      ...
      <div class="veh-add-form">
        ...
        <input type="text" id="vehNewLiters" placeholder="Litres" inputmode="none" style="display:none;">
      ...
      ${entries.length ? entries.map(e => {
        ...
        const perLiter = (e.type === 'carburant' && liters > 0 && cost > 0) ? `${(cost/liters).toFixed(3)} \u20ac/L` : '';
        return `
        <div class="veh-entry">
          <span class="veh-due-icon">${t.icon}</span>
          <div class="veh-due-body">
            <div class="veh-due-name">${t.label}${e.note ? ' \u2014 ' + esc(e.note) : ''}</div>
            <div class="veh-due-meta">${fmtDate(e.date)}${e.km ? ' \u00b7 ' + parseInt(e.km,10).toLocaleString('fr-FR') + ' km' : ''}${liters > 0 ? ' \u00b7 ' + liters.toFixed(1) + ' L' : ''}${perLiter ? ' \u00b7 ' + perLiter : ''}</div>
          </div>
  ...
  function bindVehicleEvents(v){
    ...
    ['vehNewKm','vehNewCost','vehNewLiters'].forEach(id => {
      const el = document.getElementById(id);
      if(el) ['focus','click'].forEach(ev => el.addEventListener(ev, () => openNumPad(el)));
    });

    const typeEl = document.getElementById('vehNewType');
    const litersEl = document.getElementById('vehNewLiters');
    if(typeEl && litersEl){
      typeEl.addEventListener('change', () => {
        litersEl.style.display = typeEl.value === 'carburant' ? '' : 'none';
      });
    }

    const rangeEl = document.getElementById('vehFuelRange');
    if(rangeEl) rangeEl.addEventListener('change', () => {
      vehicleFuelRange = rangeEl.value;
      renderVehicleMain();
    });

    const granEl = document.getElementById('vehFuelGranularity');
    if(granEl) granEl.addEventListener('change', () => {
      vehicleFuelGranularity = granEl.value;
      renderVehicleMain();
    });
    ...
    if(addBtn) addBtn.addEventListener('click', () => {
      ...
      const liters = (document.getElementById('vehNewLiters').value||'').replace(',','.');
      const note = document.getElementById('vehNewNote').value.trim();
      v.entries = v.entries || [];
      v.entries.push({ id: uid(), type, date: new Date(date + 'T12:00:00').toISOString(), km, cost, liters: type === 'carburant' ? liters : '', note });
