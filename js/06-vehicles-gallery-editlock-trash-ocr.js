  ...
  function renderFuelPriceChartSvg(points){
    ...
    const dateLabels = labelIdxs.map(i => `<text x="${x(i).toFixed(1)}" y="${H-8}" text-anchor="middle" font-size="10" fill="var(--text-dim)">${fmtDate(points[i].date)}</text>`).join('');

    const linePoints = points.map((p,i) => `${x(i).toFixed(1)},${y(p.pricePerLiter).toFixed(1)}`).join(' ');
    const dots = points.map((p,i) => `<circle cx="${x(i).toFixed(1)}" cy="${y(p.pricePerLiter).toFixed(1)}" r="3.5" fill="var(--gold)" stroke="var(--paper-raised)" stroke-width="1.5"><title>${fmtDate(p.date)} : ${p.pricePerLiter.toFixed(3)} €/L (${p.liters.toFixed(1)} L)</title></circle>`).join('');

    return `
      <svg viewBox="0 0 ${W} ${H}" class="fuel-price-svg" role="img" aria-label="Évolution du prix du carburant au litre">
        ${gridLines}
        <polyline points="${linePoints}" fill="none" stroke="var(--sage)" stroke-width="2.5" stroke-linejoin="round" stroke-linecap="round"/>
        ${dots}
        ${dateLabels}
      </svg>`;
  }
  ...
  function renderFuelPriceSection(v){
    ...
    if(points.length < 2){
      return `
        <div class="dash-section-title fuel-chart-title-row">⛽ Évolution du prix du carburant ${rangeSelect}</div>
    ...
    const avg = points.reduce((s,p)=>s+p.pricePerLiter,0) / points.length;
    const trend = last.pricePerLiter - first.pricePerLiter;
    ...
    return `
      ...
      <div class="fuel-price-chart">
        ${renderFuelPriceChartSvg(points)}
        <div class="fuel-price-stats">
          <span>Moyenne : <strong>${avg.toFixed(3)} €/L</strong></span>
          <span>Dernier plein : <strong>${last.pricePerLiter.toFixed(3)} €/L</strong></span>
          <span class="${trendClass}">${trendArrow} ${trend >= 0 ? '+' : ''}${trend.toFixed(3)} €/L sur la période affichée</span>
        </div>
      </div>`;
  }
