...
const CACHE_VERSION = 'rf-shell-v2'; // v2 : index_1.html renomme en index.html

// Note : "./" n'est pas dans la liste — le gestionnaire fetch ci-dessous sert index.html
// en cache pour TOUTE navigation hors-ligne (y compris la racine), donc ce n'est pas
// necessaire ici meme si l'hebergement resout deja "/" vers index.html.
const APP_SHELL = [
  './index.html',
  './manifest.json',
  './icon-192.png',
  ...
  if(isNavigation || isAppShellFile){
    ...
    event.respondWith(
      fetch(req).then(res => {
        ...
        caches.open(CACHE_VERSION).then(cache => cache.put(isNavigation ? './index.html' : req, copy));
        return res;
